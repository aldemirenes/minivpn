...
#define SSL_SERVER_RSA_CERT	"/vagrant/cert.pem"
#define SSL_SERVER_RSA_KEY	"/vagrant/key.pem"
...
int main(void)
{
	...
	if (bind(serversocketfd, (struct sockaddr*) &local, sizeof(local)) < 0) {
		perror("bind()");
		exit(1);
	}
	...
	{
		SSL *serverssl;
		char buffer[1024] = "Greetings from server";
		char buffer2[1024];
		int bytesread = 0;
		int addedstrlen;
		int ret;
		...
		bytesread = SSL_read(serverssl, buffer2, sizeof(buffer));
